...
function StudyHub() {
  ...
  // Slide indices for arrow navigation
  ...
  // Fixed numerical order index variables to guarantee linear tracking
  async function verifyLocalSyllabusSeeding() {
    const subjectCount = await db.subjects.count();
    if (subjectCount === 0) {
      console.log("Empty catalog discovered. Injecting strict sequential order elements into IndexedDB...");
  ...
  const handleSubtaskComplete = async (task, subtask) => {
    try {
      // 1. Fire database operation pass
      await completeTaskService(task.id, subtask.subtopicId, subtask.topicId);
      
      // 2. IMMEDIATE LOCAL STATE SYNC: Check if this was the last pending subtask in the carousel
      setTasks(prevTasks => 
        prevTasks.map(t => {
          if (t.id !== task.id) return t;
          
          // Add newly completed subtopic ID to our local set imitation
          const updatedCompletedSet = new Set(completedSubtopicIds);
          updatedCompletedSet.add(subtask.subtopicId);
          
          // Verify if ALL subtasks under this parent card are now finished
          const allSubtasksDone = t.subtasks?.every(st => updatedCompletedSet.has(st.subtopicId));
          
          return allSubtasksDone ? { ...t, status: "COMPLETED" } : t;
        })
      );

      // 3. Keep background registries fully aligned
  ...
  const handleTaskComplete = async (task) => {
    try {
      // 1. Fire database operation pass
      await completeTaskService(task.id, null, null);
      
      // 2. IMMEDIATE LOCAL STATE SYNC: Force parent block to COMPLETED status instantly
      setTasks(prevTasks => 
      ...
      console.error("Could not close active task:", err);
    }
  };
    ...
    try {
      ...
      for (const task of tasks) {
        if (task.status?.toUpperCase() === "COMPLETED") {
          await db.schedule_tasks.update(task.id, { status: "closed" });
        } else {
          await db.schedule_tasks.update(task.id, { status: "missed", closedAt: Date.now() });
  ...
  const handleReassembleCapacity = async () => {
    const choice = prompt("Where should we allocate the extra hours? Enter: 'GS', 'OPTIONAL', or 'BOTH'").toUpperCase();
    if (!["GS", "OPTIONAL", "BOTH"].includes(choice)) {
      alert("Invalid selection. Operation canceled.");
      return;
    }
    try {
      setLoading(true);
      const minutesToAppend = sessionHours * 60;
      await generateDailySchedule(user.uid, minutesToAppend, choice);
      alert(`Successfully appended +${sessionHours} hours to your ${choice} workflow targets!`);
      await loadTodaySchedule();
    } catch (err) {
  ...
  if (loading) {
    return <div className="py-12 text-center text-xs text-slate-400 font-mono animate-pulse">Assembling daily operational tracks...</div>;
  }
  ...
  const isGsSlotDone = isMidnightLockActive || gsTask?.status?.toUpperCase() === "COMPLETED";
  const isOptionalSlotDone = isMidnightLockActive || optionalTask?.status?.toUpperCase() === "COMPLETED";
  const isRevisionSlotDone = isMidnightLockActive || revisionTask?.status?.toUpperCase() === "COMPLETED";
  const isPracticeSlotDone = isMidnightLockActive || practiceTask?.status?.toUpperCase() === "COMPLETED";

  // EXTENSION VERIFICATION CONSTRAINTS
  const currentHour = new Date().getHours();
  const hoursUntilMidnight = 24 - currentHour;
  const maxAllowedExtensionHours = Math.min(3, hoursUntilMidnight);

  const hasActiveTasks = tasks.length > 0;
  const allTasksCompleted = hasActiveTasks && tasks.every(t => t.status?.toUpperCase() === "COMPLETED");
  const canExtendSession = allTasksCompleted && maxAllowedExtensionHours > 0 && !isMidnightLockActive;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 border border-slate-200 p-4 rounded-2xl shadow-inner">
        ...
        {/* SLOT 2: Optional Tasks Carousel Pager Card */}
        ...
        <div className={`p-4 bg-white border rounded-xl shadow-sm flex flex-col justify-between border-slate-200 transition-all ${isRevisionSlotDone ? "opacity-40 bg-slate-50/60 pointer-events-none" : ""}`}>
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-amber-600 uppercase tracking-wider">Revision Tasks</span>
              <span className="text-[10px] bg-amber-50 border border-amber-200 text-amber-700 font-bold px-2.5 py-0.5 rounded-full">
                {revisionTask && !isMidnightLockActive ? `${revisionTask.estimatedMinutes} Mins` : "--"}
              </span>
            </div>
            {revisionTask && !isMidnightLockActive ? (
              <div className="mt-2 space-y-0.5 text-left">
                <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-tight truncate">{revisionTask.subjectName}</span>
                <span className="text-sm font-black text-slate-800 block leading-tight line-clamp-1">{revisionTask.topicName}</span>
                <p className="text-xs text-slate-600 mt-1 line-clamp-2">{revisionTask.subtopicName}</p>
              </div>
            ) : (
              <p className="text-xs text-slate-500 font-bold italic py-3 text-left">✓ Block Finished.</p>
            )}
          </div>
          {revisionTask && !isMidnightLockActive && (
            <div className="mt-4 pt-2 border-t border-slate-100 flex justify-end">
              <button disabled={isRevisionSlotDone} onClick={() => handleTaskComplete(revisionTask)} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors shadow-sm ${isRevisionSlotDone ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-slate-900 text-white hover:bg-slate-800"}`}>
                {isRevisionSlotDone ? "✓ Completed" : "Mark Complete"}
              </button>
            </div>
          )}
        </div>
        ...
        <div className={`p-4 bg-white border rounded-xl shadow-sm flex flex-col justify-between border-slate-200 transition-all ${isPracticeSlotDone ? "opacity-40 bg-slate-50/60 pointer-events-none" : ""}`}>
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-emerald-600 uppercase tracking-wider">Practice Tasks</span>
            ...
            {practiceTask && !isMidnightLockActive ? (
              <div className="mt-2 space-y-0.5 text-left">
                ...
                <span className="text-sm font-black text-slate-800 block leading-tight mt-1 line-clamp-1">{practiceTask.topicName}</span>
                <p className="text-xs text-slate-600 mt-1 line-clamp-2">{practiceTask.subtopicName}</p>
              </div>
            ) : (
              <p className="text-xs text-slate-500 font-bold italic py-3 text-left">✓ Block Finished.</p>
            )}
          ...
          {practiceTask && !isMidnightLockActive && (
            <div className="mt-4 pt-2 border-t border-slate-100 flex justify-end">
              <button disabled={isPracticeSlotDone} onClick={() => handleTaskComplete(practiceTask)} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-colors shadow-sm ${isPracticeSlotDone ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-slate-900 text-white hover:bg-slate-800"}`}>
                {isPracticeSlotDone ? "✓ Completed" : "Mark Complete"}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="sm:col-span-2 grid grid-cols-2 gap-3 pt-2">
          <button disabled={isMidnightLockActive} className="py-2.5 bg-white border border-slate-200 text-xs font-bold text-slate-600 rounded-xl hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors shadow-sm">
            Regenerate Schedule
          </button>
          <button 
      ...
      {/* EXTEND SESSION AREA */}
      <div className={`bg-white border border-slate-200 p-4 rounded-xl space-y-3 shadow-sm transition-all ${!canExtendSession ? "opacity-35 bg-slate-50/50 pointer-events-none select-none shadow-none" : ""}`}>
        <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          />
          <span className="text-xs font-black text-cyan-700 bg-slate-50 border border-slate-200 px-3 py-1 rounded-md shadow-sm">+{sessionHours} Hour{sessionHours > 1 ? "s" : ""}</span>
        </div>
        <button 
          disabled={!canExtendSession}
