  ...
  return (
    ...
    >
      ...
      <div className="space-y-1.5 max-h-[60vh] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full">
        {optionalSequence && optionalSequence.length > 0 ? (
          optionalSequence.map((chapter, index) => {
            // Create a completely distinct unique key using subject name, item unique identifier, and item index 
            const uniqueKey = `${optionalSubject}-${chapter.id || chapter.name}-${index}`;
            return (
              <div
                key={uniqueKey}
          ...
          <div className="text-center py-8 text-xs text-slate-500">
            No active chapters found for {optionalSubject || "the selected subject"}.
          </div>
        )}
      </div>
